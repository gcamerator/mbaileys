...
const generateWAMessageContent = async (message, options) => {
    ...
    else if ('pollV5' in message ||'quiz' in message || 'pollCreationMessageV5' in message) {
    const poll = message.pollV5 || message.quiz || message.pollCreationMessageV5; 
        m.messageContextInfo = {
         messageSecret: (0, crypto_1.randomBytes)(32),
        };
        const pollCreationMessage = {
            name: poll.name || poll.question || poll.text,
            options: poll.options ? poll.options.map(name => ({ optionName: name })) : poll.values.map(name => ({ optionName: name }))
        };
       m.pollCreationMessageV5 = {
                    // Lia@Note 08-02-26 --- quiz for newsletter only
                    ...pollCreationMessage,
                    correctAnswer: {
                        optionName: poll.correctAnswer.toString() || poll.answer.toString()
                    },
                    pollType: 1,
                    selectableOptionsCount: 1
                };
